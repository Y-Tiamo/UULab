...
export default class Select extends Component {
  ...
  showPullPicker() {
    ...
    PullPicker.show(pickerTitle, items, this.selectedIndex, onSelected, { getItemText });
  }

  showPopoverPicker() {
    this.measure((x, y, width, height, pageX, pageY) => {
      let { items, getItemText, onSelected } = this.props;
      PopoverPicker.show({ x: pageX, y: pageY, width, height }, items, this.selectedIndex, onSelected, {
        getItemText,
        align: 'end',
      });
    });
  ...
  renderIcon() {
    ...
    return (
      <View style={{ position: 'absolute', top: 0, bottom: 0, right: 0, justifyContent: 'center' }}>{iconElement}</View>
    );
  }
