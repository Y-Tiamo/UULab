...
export default class NavigationBar extends Component {
  ...
  renderStatusBar(fs) {
    ...
    if (!statusBarColor)
      statusBarColor =
        statusBarInsets && (Platform.OS === 'ios' || Platform.Version > 20) ? 'rgba(0,0,0,0)' : fs.backgroundColor;
  ...
  renderTitle(fs) {
    ...
    if (typeof title === 'string') {
      ...
      title = (
        <this.constructor.Title style={[{ textAlign, color: Theme.navTitleColor }].concat(titleStyle)} text={title} />
      );
    }
  ...
  renderRightView() {
    ...
    let { barOpacity: opacity } = this.state;
    return (
      <Animated.View style={{ opacity }} onLayout={e => this.onRightViewLayout(e)}>
        {rightView}
      </Animated.View>
    );
  }
